...
def count_tokens(text: str, model: str = "deepseek-v4-flash") -> int:
    """计算文本的 token 数量"""
    try:
...
def count_messages_tokens(messages: List[Dict[str, str]], model: str = "deepseek-v4-flash") -> int:
    """计算消息列表的总 token 数"""
    total = 0
    for msg in messages:
...
摘要："""
        ...
        async for chunk in llm_service.stream_chat(
            ...
            api_key=api_key,
            model="deepseek-v4-flash",  # 使用 DeepSeek V4 Flash 生成摘要
            use_rag=False,
            use_memory=False
        ):
